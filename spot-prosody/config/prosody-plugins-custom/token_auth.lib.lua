-- Module for verifying tokens. Adapted from token/util.lib.lua (in jitsi-meet) and mod_auth_spot_token
...
function Util:process_and_verify_token(session, acceptedIssuers)
    ...
    if claims ~= nil then
        if self.requireRoomClaim then
            local roomClaim = claims["spotRoomId"];
        ...
        session.spot_room = claims["spotRoomId"];
        -- Binds device ID to the session
        session.spot_device = claims["spotDeviceId"];

        -- Binds the user details to the session if available
        if claims["context"] ~= nil then
          if claims["context"]["user"] ~= nil then
            session.spot_context_user = claims["context"]["user"];
          end

          if claims["context"]["group"] ~= nil then
            -- Binds any group details to the session
            session.spot_context_group = claims["context"]["group"];
          end

          if claims["context"]["features"] ~= nil then
            -- Binds any features details to the session
            session.spot_context_features = claims["context"]["features"];
          end
          if claims["context"]["room"] ~= nil then
            session.spot_context_room = claims["context"]["room"]
          end
        end
        return true;
    else
...
--         if session.spot_context_room and (session.spot_context_room["regex"] == true or session.spot_context_room["regex"] == "true") then
--             if target_room ~= nil then
--                 -- room with subdomain
