...
class UpdateController {
     ...
     * Initializes.
     *
     * @param spotClientController - The client controller used to listen for commands sent by the spot-client.
     */
    constructor(spotClientController: typeof clientController) {
        ...
        spotClientController.on('spot-electron/auto-updater', ({ updateAllowed }: { updateAllowed: boolean; }) => {
     ...
     * @returns {void}
     */
    checkForUpdateAndStartDownload(): void {
        if (isDev) {
        ...
        autoUpdater.checkForUpdates().then(result => {
            logger.info('Check for update results', { updateInfo: result?.updateInfo });
        }, error => {
     ...
     *
     * @param updateInfo - Any info about the update as specified by the electron-updater API.
     * @private
     * @returns {void}
     */
    private _onUpdateDownloaded(updateInfo: unknown): void {
     ...
     * @returns {void}
     */
    private _updateAndRestart(): void {
        const isSilent = true;
...
export default new UpdateController(clientController);
