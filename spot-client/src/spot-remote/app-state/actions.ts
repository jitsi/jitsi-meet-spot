 ...
 * @type {Array<Function>}
 */
let rcsListeners: Array<() => void> = [];

/**
 * Connects Spot Remote to Spot TV.
 *
 * @param joinCode - The join code to be used by Spot Remote in order to be paired with a Spot TV.
 * @param shareMode - Indicates whether the Spot Remote is running in the special screen share mode or not.
 * @returns {Function}
 */
export function connectToSpotTV(joinCode: string, shareMode: boolean) {
    return (dispatch: any, getState: any) => {
         ...
         *
         * @param roomProfile - Information about the room to which the Spot Remote has been connected to.
         * @returns {void}
         */
        function onSuccessfulConnect(roomProfile: any) {
            dispatch({
            ...
            if (roomId) {
                ...
                dispatch(setSpotInstanceInfo({
                    roomId,
                    isSpotTv: false,
                    isPairingPermanent: Boolean(backend && backend.isPairingPermanent())
                }));
            }
         ...
         * @returns {Promise}
         */
        function onDisconnect(error: Error | string) {
        ...
        const onBeforeUnloadHandler = (event: BeforeUnloadEvent) => dispatch(disconnectFromSpotTV(event));
 ...
 *
 * @param joinCode - The join code recived.
 * @returns {Object}
 */
export function setAPiReceivedJoinCode(joinCode: string) {
    return {
 ...
 * screen.
 *
 * @param mostRecentCountryCode - The country code returned by the country picker.
 * @returns {Object}
 */
export function setMostRecentCountryCode(mostRecentCountryCode: string) {
    return {
 ...
 * @param data - Details of the Spot-TV's current state.
 * @private
 * @returns {void}
 */
function _onSpotTVStateChange({ dispatch }: { dispatch: any; }, data: any) {
 ...
 * @returns {Function}
 */
export function disconnectFromSpotTV(event?: any) {
    return (dispatch: any) => {
        _clearSubscriptions();
 ...
 * @param willRetry - Indicates whether or not Spot Remote will try to reconnect.
 * @param otherFlags - Other flags that affect the {@code willRetry}.
 * @returns {Object}
 */
function spotRemoteConnectionFailed(
        { error, willRetry, ...otherFlags }:
        { error: Error | string; willRetry: boolean; [key: string]: any; }) {
    return {
 ...
 *
 * @param password - The meeting password to use.
 * @returns {Function}
 */
 ...
 *
 * @param remoteJoinCode - The Spot-TV join code that should be used
