...
interface IProps {
    animationRevealRate?: number;
    animationStartDelay?: number;
    domain?: string;
    onSubmit?: (meetingToJoin: string) => void;
    t?: (key: string) => string;
    tenant?: string;
}

interface IState {
    animatingPlaceholder: string;
 ...
 */
export class SelfFillingNameEntry extends React.Component<IProps, IState> {
    static defaultProps = {
        animationRevealRate: 100,
        animationStartDelay: 6000
    };

    _animationInterval: ReturnType<typeof setInterval> | null;
    _fullGeneratedMeetingName: string;
    _generateRoomNameTimeout: ReturnType<typeof setTimeout> | null;
    ...
    _clearRandomMeetingNameProcess() {
        ...
        if (this._generateRoomNameTimeout) {
     ...
     *
     * @param enteredMeetingName - The meeting name as it has been
     * typed into the input.
     * @private
     * @returns {void}
     */
    _onChange(enteredMeetingName: string) {
