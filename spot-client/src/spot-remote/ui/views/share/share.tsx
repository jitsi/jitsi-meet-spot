...
interface IProps {
    ...
    history?: any;
    isConnectedToSpot?: boolean;
    isScreenshareActiveRemotely?: boolean;
    isWirelessScreensharing?: boolean;
    isWirelessScreensharingPending?: boolean;
    meetingUrl?: string;
    t: (key: string) => string;
}

interface IState {
    autoPromptScreenshare: boolean;
}

/**
 * Controls what UI components display while in share mode.
 */
export class Share extends React.PureComponent<IProps, IState> {
    /**
    ...
    render() {
        return (
            <View name = 'share-view'>
                <WithRemoteControl
                    { ...({ disconnectOnUnmount: false } as any) } >
     ...
     */
    _getWirelessScreenshareDisabledText(): string | undefined {
        const { t } = this.props;
