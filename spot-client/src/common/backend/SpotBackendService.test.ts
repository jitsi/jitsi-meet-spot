...
jest.mock('common/utils', () => {
    return {
        ...(jest.requireActual('common/utils') as any),
 ...
 * @returns
 */
function refreshRequestMatcher(refreshToken: string): any {
 ...
 *
 * @param spotBackendService - The backend instance to run the scenario with.
 * @returns
 */
function checkRetryOn401ToGetRoomInfo(spotBackendService: any): Promise<void> {
    ...
    return spotBackendService.getRoomInfo().then((roomInfo: any) => {
            ...
            it('re-uses endpoint ID received on the 1st register', () => {
                ...
                return spotBackendService.register(MOCK_PAIRING_CODE)
                    .then(() => {
                        ...
                        peristenceSpy.mockImplementation((key: any) => {
