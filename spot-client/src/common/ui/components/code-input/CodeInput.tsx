...
export default class CodeInput extends React.Component<IProps, IState> {
     ...
     * @returns {void}
     */
    _onChange(event: React.ChangeEvent<HTMLTextAreaElement>) {
     ...
     *
     * @param event - The keyboard event itself.
     * @private
     * @returns {void}
     */
    _onKeyDown(event: React.KeyboardEvent<HTMLTextAreaElement>) {
    ...
    _renderBoxes() {
        ...
        const focus = Math.min(this.state.value.length, length - 1);
        const boxes = [];
