...
interface IPostToEndpointOptions {
    deviceId: string;
    endpointUrl: string;
}
...
export default class PostToEndpoint {
    ...
    private _requestInFlight: PostLogsRequest | null;
    private _throttledSend: () => void;

    /**
     * Initializes a new {@code PostToEndpoint} instance.
     *
     * @param options - Configuration for how the instance should send
     * logs.
     * @param deviceId - A unique identifier for this client.
     * @param endpointUrl - The URL for which to send POSTs.
     */
    constructor({ deviceId, endpointUrl }: IPostToEndpointOptions) {
     ...
     *
     * @param events - The log statements with meta data. The
    ...
    send(events: any[]): void {
        ...
        const jsonEvents = events.reduce((accumulator: any[], event: any) => {
            try {
