...
describe('PostLogsRequest', () => {
    ...
    let postLogsRequest: PostLogsRequest;

    beforeEach(() => {
        jest.useFakeTimers({ advanceTimers: true });

        postLogsRequest = new PostLogsRequest(testEvents as any, testDeviceId, testEndpoint);
    });

    describe('send without error', () => {
        it('resolves on success', () => {
            (fetch as any).mockResponse(() => Promise.resolve({}));
    ...
    describe('retry', () => {
        it('retries on failed send', () => {
            (fetch as any).mockResponses(
                [ JSON.stringify({}), { status: 500 } ], [ JSON.stringify({}), { status: 200 } ]);
        ...
        it('retries at least MAX_RETRIES times', () => {
            ...
            (fetch as any).mockResponses(...responses);
