 ...
 *
 * @param oldState - The outdated state object.
 * @param newState - The new state object to be compared to the old
 * state object.
 * @private
 * @returns True if the new state has a changed value that should
 * trigger a persistence update.
 */
function hasUpdateOfInterest(oldState: any, newState: any): boolean {
    return keysToStore.some(key => {
        const statePath = key.split('.');
        const oldValue = statePath.reduce((a: any = {}, b) => a[b], oldState);
        const newValue = statePath.reduce((a: any, b) => a[b], newState);
 ...
 *
 * @param state - The state object from Redux.
 * @private
 * @returns A new state object formatted for persisted storage.
 */
function parsePersistedState(state: any): any {
    return {
        backend: {
 ...
 * @returns
 */
export function getPersistedState(): any {
 ...
 * @returns {void}
 */
export function setPersistedState(store: any): void {
