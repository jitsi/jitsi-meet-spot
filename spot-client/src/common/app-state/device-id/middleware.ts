 ...
 *
 * @param roomId - The room ID assigned by the backend.
 * @param isSpotTv - Spot TV or Spot Remote ?
 * @param isPairingPermanent - Is it permanent(long lived) backend pairing.
 * @returns
 * @private
 */
function _getDeviceIdPrefix(roomId: string, isSpotTv: boolean, isPairingPermanent: boolean): string {
    const middlePart
        = isSpotTv
            ? 'spot-tv'
            : isPairingPermanent
