...
MiddlewareRegistry.register(() => (next: any) => (action: any) => {
 ...
 * @returns {void}
 */
function _sendApiMessage(messageType: string, messageData: string | object): void {
    let target: { postMessage: (message: string) => void; } | undefined;

    // React native webview
    // eslint-disable-next-line prefer-const
    target = target || (window as any).ReactNativeWebView;
