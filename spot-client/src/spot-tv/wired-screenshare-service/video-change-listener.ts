...
export default class VideoChangeListener extends Emitter {
    _deviceLabel: string;
    _initialFrameValue: number | undefined;
    _videoElement: HTMLVideoElement;
    _context: CanvasRenderingContext2D | null;
    _jitsiLocalTrack: any;
    _diffCheckInterval: ReturnType<typeof setInterval> | undefined;
    _isDeviceConnected: boolean;
    _startPromise: Promise<void> | undefined;

    /**
     * Initializes a new {@code VideoChangeListener} instance.
     *
     * @param deviceLabel - The label associated with the video input
     * device to be monitored. The label is likely obtained from a call to
     * enumerateDevices.
     * @param initialFrameValue - The rgba sum to use as a baseline
     ...
     */
    constructor(deviceLabel: string, initialFrameValue?: number) {
        super();
         ...
         * @type {IntervalID}
         */
        this._diffCheckInterval = undefined;
     ...
     * idle frame.
     *
     * @param callback - Callback to invoke when there is video
     ...
     *
     * @param callback - The callback which should no longer be
     * notified.
     * @returns {void}
     */
     ...
     * @returns {Promise}
     */
    _getVideoStream(): Promise<any> {
        let desiredDeviceId: string;

        return avUtils.enumerateDevices()
            .then((devices: any[]) => devices.find(
                (device: any) => device.label === this._deviceLabel
     ...
     *
     * @param isDeviceConnected - Whether or not the change indicates
     * a device has been connected or disconnected.
     * @private
     * @returns {void}
     */
    _notifyChangeDetected(isDeviceConnected: boolean): void {
