...
export default class ApiHealthCheck {
    ...
    _healthCheckFunction: () => Promise<any>;
    _onError: (reason?: any) => void;
    _isRunning: boolean;
    _checkHealthTaskHandle?: ReturnType<typeof setTimeout>;

    /**
     * Initializes a new {@code ApiHealthCheck} instance.
     *
     * @param healthCheckFunction - The function to invoke that checks
     * the health of the iframe.
     * @param onError - Callback to invoke when a health check fails.
     */
    constructor(healthCheckFunction: () => Promise<any>, onError: (reason?: any) => void) {
