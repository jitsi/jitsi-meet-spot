import { SimpleSelect } from 'common/ui';
import React from 'react';
import { withTranslation } from 'react-i18next';


/**
 * The type of the React {@code Component} props of {@link MediaSelector}.
 */
interface IProps {
    device?: string;
    devices?: any[];
    id?: string;
    label?: string;
    onChange?: (value: string) => void;
    qaId?: string;
    t?: (key: string) => string;
    type?: string;
}

/**
 * Displays a select element for choosing a media device.
 */
export class MediaSelector extends React.Component<IProps> {
    /**
     * Initializes a new {@code MediaSelector} instance.
     *
     * @param props - The read-only properties with which the new
     * instance is to be initialized.
     */
    constructor(props: IProps) {
        super(props);

        this._onChange = this._onChange.bind(this);
    }

    /**
     * Implements React's {@link Component#render()}.
     *
     * @inheritdoc
     */
    render() {
        const {
            id,
            devices,
            device,
            label: selectorLabel,
            t
        } = this.props;

        return (
            <div
                className = 'selector'
                id = { id }>
                <div className = 'select-label'>{ selectorLabel }</div>
                <SimpleSelect
                    onChange = { this._onChange }
                    options = { devices ?? [] }
                    placeholder = { t?.('setup.selectDevice') }
                    qaId = { this.props.qaId }
                    value = { device } />
            </div>
        );
    }

    /**
     * Callback fired when a media device has been selected.
     *
     * @param value - The value from the the selected option.
     * @private
     * @returns {void}
     */
    _onChange(value: string) {
        this.props.onChange?.(value);
    }
}

export default withTranslation()(MediaSelector);
