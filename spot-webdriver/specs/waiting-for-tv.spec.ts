...
describe('The "waiting for the Spot TV to connect" message', () => {
    ...
    it('should appear if no Spot TV and disappear when it connects', async (): Promise<void> => {
    ...
    it('should appear if Spot TV disconnects while Spot Remote is disconnected', async (): Promise<void> => {
        if (!SpotSession.isBackendEnabled()) {
            pending();
