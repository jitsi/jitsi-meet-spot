...
export interface SpotWebviewProps {
    url?: string;
}

interface SpotWebviewState {
 ...
 */
export default class SpotRemoteWebView extends React.PureComponent<SpotWebviewProps, SpotWebviewState> {
    ...
    _preventWebViewZoomScript: string = `
     ...
     * @inheritdoc
     */
    componentDidUpdate(prevProps: SpotWebviewProps) {
     ...
     * page.
     *
     * @param syntheticEvent - The error event from loading the url.
     * @private
     * @returns {void}
     */
    _onWebViewLoadError(syntheticEvent: WebViewErrorEvent) {
        this.setState({
    ...
    async _resolveUrl() {
        ...
        try {
            const response = await fetch(this.props.url as string, { method: 'HEAD' });
     ...
     *
     * @param ref - The web view reference.
     * @returns {void}
     */
    _setRef(ref: WebView | null) {
        this._ref = ref;
